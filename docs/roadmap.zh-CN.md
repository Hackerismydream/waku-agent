...
| 执行轨迹级 Eval | 部分完成 | 194 项离线确定性检查；40 条候选任务固定为 24 条开发集与 16 条留出集；DeepSeek V4-Pro 完成 40×3 次真实运行，V4-Flash Judge 120/120 可用；审计后端到端通过率 95.0%，37/40 个任务 3/3 | 作者逐条复核任务并批准 manifest；再补 2 个候选模型形成 3 模型 × 3 次冻结 baseline |
...
- 已写入 40 条候选任务，结构化断言覆盖工具路径、参数、检索门控、工具结果和沙箱终态，开发集/留出集固定为 24/16。
- 已实现 `provider:model × task × trial` 矩阵运行器；每次尝试保存 Git、数据集与 Prompt 版本、控制决策、完整工具调用、最终回复、token、延迟、估算成本、Judge 结果、状态快照、trace 和 verdict。
- 已实现七类失败归因、每次运行的 `failures.jsonl` 和跨运行追加的 `regressions.jsonl`。
- release gate 在无 Judge key 时输出 `GATE CONDITIONAL`，不再声称完整语义门禁通过。
- 已使用 DeepSeek V4-Pro 对全部 40 条任务各运行 3 次，并使用 V4-Flash 完成 120 次 Judge；源运行 0 个执行错误，原始端到端通过 112/120，校正两条合同假阴性后对同一 receipts 审计重放为 114/120，37/40 个任务保持 3/3。
- 真实失败集中在消息草稿遗漏/重复副作用、硬字符上限和恢复时误写长期记忆；完整口径见 `docs/eval-results/2026-07-19-deepseek-v4-pro.md`。
- 尚未完成：`evals/dataset_manifest.json` 仍为 `pending_author_review`；在作者逐条复核前，`make baseline` 会拒绝冻结报告。当前只有 1 个候选模型完成 3 次重复，没有生成或冒充 3 模型冻结 baseline。
